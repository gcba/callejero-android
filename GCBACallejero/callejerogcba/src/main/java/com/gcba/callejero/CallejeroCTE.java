package com.gcba.callejero;

/**
 * Created by julian on 15/08/17.
 */

public class CallejeroCTE {

    public static final String URL_LATLONG = "https://ws.usig.buenosaires.gob.ar/geocoder/2.2/reversegeocoding";
    public static final String URL_NORMALIZE = "https://servicios.usig.buenosaires.gob.ar/normalizar";
    public static final String URL_EPOK_BUENOSAIRES_GOB_AR_BUSCAR = "https://epok.buenosaires.gob.ar/buscar";
    public static final String URL_EPOK_BUENOSAIRES_GOB_AR_GET_OBJECT_CONTENT = "https://epok.buenosaires.gob.ar/getObjectContent";

    public static final String STANDARDIZED_ADDRESS_DATA = "_STANDARDIZED_ADDRESS_DATA_";
    public static final String SHOW_ADDRESS_FROM_CABA = "_SHOW_ADDRESS_FROM_CABA_";
    public static final String SHOW_PIN = "_SHOW_PIN_";
    public static final String SHOW_LABEL = "_SHOW_LABEL_";
    public static final String SHOW_PLACES = "_SHOW_PLACES_";

    public static final String PLACE = "_PLACE_";

    //Convención USIG
    public static final String CALLE = "calle";
    public static final String CALLE_ALTURA = "calle_altura";
    public static final String CALLE_CALLE = "calle_y_calle";
    public static final String CALLE_ALTURA_CALLE_CALLE = "calle_altura_calle_y_calle";

}
